...
### 3.1 메모리 최적화 및 SGBM 적용
자원 한계를 극복하기 위해 ZRAM 스왑을 적용하고 연산 해상도를 320x240으로 제한하였습니다. 깊이 맵의 텍스처 소실 현상을 개선하기 위해 SGBM(Semi-Global Block Matching) 3-Way 모드를 적용하였습니다.

### 3.2 투영 오차 보정 (Geometric Calibration)
...
### 3.3 데이터 안정화 필터링
저해상도 연산 시 발생하는 거리 수치의 요동(Jittering) 현상을 제어하기 위해 관심 영역(ROI)의 중앙값(Median)을 추출하고, 지수 이동 평균(EMA) 필터를 적용하여 데이터를 평활화(Smoothing)하였습니다.

### 3.4 객체 면적 산출 (Dimension Estimation)
...
* **라즈베리파이3 최적화 가이드:** 본 시스템의 구동을 위한 메모리 최적화 설정은 반드시 다음 링크를 참조하여 적용하시기 바랍니다.  
  👉 [Raspberry Pi 3 OS 및 메모리 튜닝 가이드](https://github.com/akffkdzkdngmrdn87/-3-Rev1.3/blob/main/%ED%95%84%EB%8F%85.md)
